...
import Semantics.Compiler (ilCompileProgram)
...
data ReplMode = OnlyTokens | OnlyAST | Evaluate | Compile2IL deriving (Eq, Show)

getReplModeFromArgs :: [String] -> ReplMode
getReplModeFromArgs ("-t":_) = OnlyTokens
getReplModeFromArgs ("-a":_) = OnlyAST
getReplModeFromArgs ("-c":_) = Compile2IL
...
repl m s = do
  ...
  if (m == OnlyTokens) then do
    ...
    if (m == OnlyAST) then do
      print ast
      repl m s
    else if (m == Compile2IL) then do
      let il = ilCompileProgram ast "CompiledProgram"
      putStrLn il
      repl m s
    else do
